...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await client.from('universities').update(payload).eq('id', params.id).select('*').single();
    if (result.error) throw result.error;

    return ok(result.data);
  } catch (error) {
    return fail(error instanceof Error ? error.message : '大学データの更新に失敗しました。', 400);
  }
}
